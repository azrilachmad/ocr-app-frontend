...
const defaultDocumentTypes = [
    ...
    },
    {
        ...
        fields: [
            ...
            { name: 'Tahun Pembuatan', required: true },
    ...
    },
    {
        ...
        fields: [
            ...
            { name: 'Nama Pemilik', required: true },
            { name: 'Alamat', required: true },
            { name: 'Merk/Type', required: true },
            { name: 'Jenis/Model', required: true },
            { name: 'Tahun Pembuatan', required: true },
            { name: 'No. Rangka', required: true },
            { name: 'No. Mesin', required: true },
            { name: 'Warna', required: false },
            { name: 'Isi Silinder', required: false },
            { name: 'Bahan Bakar', required: false },
            { name: 'Warna TNKB', required: false },
        ]
    }
];
...
const SettingsPage = () => {
    ...
    const toggleDocTypeExpand = (id) => {
        setDocumentTypes(prev => prev.map(dt =>
            dt.id === id ? { ...dt, expanded: !dt.expanded } : dt
        ));
    };

    const toggleDocTypeActive = (id) => {
        setDocumentTypes(prev => prev.map(dt =>
            dt.id === id ? { ...dt, active: !dt.active } : dt
        ));
    };
        ...
        if (editingDocType) {
            setDocumentTypes(prev => prev.map(dt =>
                dt.id === editingDocType.id
                    ? { ...dt, name: newDocType.name, description: newDocType.description, fields: newDocType.fields }
                    : dt
            ));
            setSnackbar({ open: true, message: 'Document type updated successfully!', severity: 'success' });
        } else {
            const newId = Math.max(...documentTypes.map(dt => dt.id)) + 1;
                                ...
                                <Box key={docType.id} sx={{ mb: 2 }}>
                                    <Box
                                        sx={{
                                            ...
                                            bgcolor: '#F9FAFB',
                                            border: '1px solid #E5E7EB',
                                            borderRadius: docType.expanded ? '8px 8px 0 0' : 2,
                                            cursor: 'pointer'
                                        }}
                                    ...
                                    >
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                            <IconButton size="small">
                                                {docType.expanded ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                            </IconButton>
                                            <Box>
                                                ...
                                                <Typography sx={{ fontSize: '12px', color: '#6B7280' }}>
                                                    {docType.description} - {docType.fields.length} fields configured
                                                </Typography>
                                            </Box>
                                        </Box>
